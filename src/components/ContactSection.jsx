...
export default function ContactSection() {
  ...
  const saveToTextFile = (formData) => {
    ...
    a.href = url;
    a.download = `contact_form_${Date.now()}.txt`;
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-slate-800 p-8 rounded-lg shadow-lg border border-slate-700">
            <div>
              ...
              {/* Error messages */}
              {errors.spam && <p className="text-red-400 text-sm mb-4">{errors.spam}</p>}
              {errors.submit && <p className="text-red-400 text-sm mb-4">{errors.submit}</p>}
              {errors.bot && <p className="text-red-400 text-sm mb-4">Bot detected. Please try again.</p>}
